...
* Un paso a la vez: Si la tarea involucra múltiples capas (Validator -> Repository -> Service -> Controller -> Routes), pregunta y confirma antes de saltar a la siguiente capa.
...
* No intentes "ahorrar pasos": Aunque parezca redundante, sigue el orden Validator -> Repository -> Service -> Controller -> Routes, a menos que exista un motivo explícito para desviarse.
...
* Identificadores puros: Los identificadores en los DTOs y respuestas JSON deben ser numéricos puros según la base de datos (ej. `idVenta: 1`, `pedidoId: 10`), sin agregar prefijos o formateos sintéticos (como `VEN-0001` o `PED-0001`) salvo solicitud expresa.
